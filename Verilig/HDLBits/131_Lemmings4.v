module top_module(
    ...
    // count logic
    always @* begin
        next_count_fall = count_fall;
        if ((state == FALL_L) | (state == FALL_R)) begin
            // ovf condition
            if (count_fall == 5'd31) begin
                next_count_fall = count_fall;
            end else begin
                next_count_fall = count_fall + 5'd1;
            end
        end
    end

    assign en_count_fall = ((state == FALL_L) | (state == FALL_R)) ? 1'b1 : 1'b0;
    ...
    always @ (posedge clk, posedge areset) begin
        if (areset) begin
            count_fall <= 5'd0;
        end else if (en_count_fall) begin
